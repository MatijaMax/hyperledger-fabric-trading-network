...
export PEER_PER_ORGANIZATION_NUMBER=3
...
export DOCKER_COMPOSE_FILE_PATH=${DOCKER_PATH}/docker-compose-peers-orderers.yaml
...
function createConsortium() {
  ...
  set -x
  configtxgen -profile ThreeOrgsOrdererGenesis -channelID system-channel -outputBlock ${SYSTEM_GENESIS_BLOCK_PATH}/genesis.block
  res=$?
  { set +x; } 2>/dev/null
  if [ $res -ne 0 ]; then
